...
-  接下来一个简单算法题：给定一个字典字符串A，一个子字符串B，问求B中的字符是否都在A中出现过的算法(不要求出现顺序)。然后把该算法写出来发给他。
    -  答：
        -  先说了**暴力法，** 对B中的字符依次遍历，去A中查找是否出现过。复杂度为<img src="http://chart.googleapis.com/chart?cht=tx&chl=O(m \times n)" style="border:none;">。<br>
        - 然后面试官说**不暴力** 怎么做，接下来就自己举了个例子，一看可以通过Hash的方式，用一个数组对A中出现过的字母进行记录（出现过记录为1，否则0），然后遍历B字符串中的字符，去数组中对应位置检验该字母是否出现过，检验失败就返回false。遍历完成就返回true。算法复杂度优化为**O(m+n)** 。
- 怎么看待加班问题？
    - 答：主要从大环境方面和自己个人看法上进行了回答，另外举了个研究生期间996的实例说明[捂脸哭]。